...
- [ ] `/watchlist` 策略看盘空态 / note 可读，文案不引导「桌面刷新」；可切换「启发式确认 / 回测双均线 / 趋势均线」；刷新后保留上次模式；「同参回测」仅预填；「入队回测」confirm 后入队并跳转 `/backtest?job_id=` 轮询（不自动因同参开跑）
...
- [ ] `/radar` 「共振展望」可展开；有 cache 时展示启发式共振行与时间；行可「自选」「草案」（无草案可建空稿）；无数据时见空态引导跑 `scan_horizon_outlook`；可「去 Ops」
- [ ] `/radar` 「规则预测」可展开；有 `rules_v1` cache 时可见预测分与理由；行可「自选」「草案」；horizon 有而 predict 无时见重跑引导；两区皆无时引导同一 job
...
- [ ] Ops 手动跑 **`warm_watchlist_strategy_cache`** 非 skipped（Redis 桥 + 启发式 v2 + double_ma + trend_ma 三轨）；message 含三轨/trend_ma；有日 K 的自选在 `/watchlist` 三模式均可见信号；强度列可见「弱/中/强」档
