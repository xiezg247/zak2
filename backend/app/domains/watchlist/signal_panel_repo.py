...
from app.domains.watchlist.schemas import SignalPanelOut
...
class SignalPanelRepository:
    ...
    def add_symbol(self, raw: str) -> list[str]:
        ...
        try:
            ...
            raise ValidationFailed(f"标的无效：{exc}") from exc
        if vt in current:
            return current
        if len(current) >= SIGNAL_PANEL_MAX_SYMBOLS:
            raise ValidationFailed(f"信号名单已满（上限 {SIGNAL_PANEL_MAX_SYMBOLS}）")
