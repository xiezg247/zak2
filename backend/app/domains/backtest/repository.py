...
from app.domains.backtest.backtest_bars import Bar, bars_to_records, load_bars
from app.domains.backtest.backtest_optimize import pick_best
from app.domains.backtest.backtest_settings import build_strategy_setting, min_bars_for_request
from app.domains.backtest.schemas import BacktestBatchOut, BacktestRunOut, BacktestRunRequest, OptimizeSummaryOut
...
class BacktestRepository(BaseRepository[BacktestRun]):
    ...
    def run_vnpy(self, req: BacktestRunRequest, bars: list[Bar]) -> dict[str, Any]:
        try:
            ...
            raise Unavailable(
                "vnpy 未安装：请使用 backtest-worker（pip/uv extra backtest）"
            ) from exc
    ...
    def execute_single(
        ...
        *,
        ...
        try:
            ...
            err = AppError(f"策略「{req.strategy}」尚未实现")
            err.status_code = 501
            raise err from exc
