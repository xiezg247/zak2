...
from app.domains.content import playbook as playbook_svc
from app.domains.content import reports
...
def get_team_report(
    ...
    return ApiResponse(data=reports.get_team_report(db, str(user.id), report_id))
...
def list_team_reports(
    ...
    rows = reports.list_team_reports(db, str(user.id), vt_symbol)
...
def list_team_reports_page(
    ...
    result = reports.list_team_reports_page(
        db, str(user.id), vt_symbol, page=page, page_size=page_size
    )
...
def delete_note_entry(
    ...
    notes_svc.delete_entry(db, str(user.id), entry_id)
