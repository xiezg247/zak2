...
from app.domains.content.schemas import FeedItemOut, FeedSubOut
...
def search_bilibili_ups(q: str, *, limit: int = 8) -> list[dict[str, str]]:
    ...
    if not cookies:
        raise ValidationFailed("未配置 BILIBILI_COOKIES")
...
def add_bilibili_up(
    ...
    *,
    ...
    if int(count or 0) >= MAX_FEED_SUBSCRIPTIONS:
        raise ValidationFailed("订阅数已达上限")
    ...
    if existing:
        raise ValidationFailed("已订阅该 UP")
    ...
    try:
        ...
        try:
            ...
            db.rollback()
            raise ValidationFailed("该 UP 已被订阅") from exc
