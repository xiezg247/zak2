...
from app.core.errors import Unavailable, ValidationFailed
from sqlalchemy import select, text
from sqlalchemy.orm import Session

from app.core.time import china_today
from app.models.market import EmotionLimitLadderDaily
from app.domains.market.schemas import EmotionSnapshot, MarketOverview, RankRow
from app.domains.emotion import emotion_cycle as emotion_cycle_svc
from app.domains.market.db_ranks import db_rank_fallback
from app.domains.market.quotes import QuoteStore, get_quote_store
...
def market_ranks(db: Session, field: str, *, top_n: int = 50) -> list[RankRow]:
    if field not in RANK_FIELDS:
        raise ValidationFailed(f"不支持的排行字段：{field}")
    ...
    if trading and not store.available():
        raise Unavailable("Redis 不可用，且数据库无收盘数据")
    return []
