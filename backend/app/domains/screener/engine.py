...
def _require_quotes(store: QuoteStore) -> None:
    if not store.available():
        raise Unavailable("Redis 不可用，请检查 REDIS_URL 或先启动 Redis")
    meta = store.meta()
    if not meta.get("quote_count"):
        raise Unavailable("行情快照为空，请启动 quote-collector（python -m app.quote_collector）")
...
def run_condition_screen(
    ...
    *,
    ...
    if preset is None:
        raise ValidationFailed(f"未知 preset：{req.preset}")
    if not preset.implemented:
        exc = AppError(f"preset「{req.preset}」尚未实现")
        exc.status_code = 501
        raise exc
...
def run_recipe_screen(
    ...
    *,
    ...
    if recipe is None:
        raise ValidationFailed(f"未知 recipe：{req.recipe_id}")
    if not recipe.implemented:
        exc = AppError(f"recipe「{req.recipe_id}」尚未实现")
        exc.status_code = 501
        raise exc
    ...
    if recipe.recipe_id == "radar_resonance":
        from app.domains.screener import resonance_screen

        if db is None or not user_id:
            raise ValidationFailed("雷达共振需要数据库与登录用户")
