...
from app.schemas.market import SectorFlowRow, SectorIntradayPoint
...
def list_sector_flow(
    ...
    *,
    ...
    if kind not in {"industry", "concept"}:
        raise HTTPException(status_code=400, detail="kind 须为 industry 或 concept")
    sort = sort.strip()
    if sort not in {"net_flow_yi", "change_pct"}:
        raise HTTPException(status_code=400, detail="sort 须为 net_flow_yi 或 change_pct")
