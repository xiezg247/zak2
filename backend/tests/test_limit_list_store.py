...
from app.domains.market.limit_list_store import attach_first_time_fields, load_first_time_map
...
def test_load_first_time_map_from_db() -> None:
    ...
    with patch("app.domains.market.limit_list_store.latest_open_yyyymmdd", return_value="20240805"):
...
def test_load_first_time_map_lazy_fetch() -> None:
    ...
    db.execute.side_effect = _execute
    with (
        patch("app.domains.market.limit_list_store.latest_open_yyyymmdd", return_value="20240805"),
        patch("app.domains.market.limit_list_store.ts.require_token", return_value="tok"),
        patch("app.domains.market.limit_list_store.sync_one_day", return_value=1) as sync_day,
    ):
...
def test_load_first_time_map_no_token_silent() -> None:
    ...
    with (
        patch("app.domains.market.limit_list_store.latest_open_yyyymmdd", return_value="20240805"),
        patch(
            "app.domains.market.limit_list_store.ts.require_token",
            side_effect=ts.TushareNotConfiguredError("未配置 TUSHARE_TOKEN"),
        ),
        patch("app.domains.market.limit_list_store.sync_one_day") as sync_day,
    ):
        out = load_first_time_map(db, lazy_fetch=True)
