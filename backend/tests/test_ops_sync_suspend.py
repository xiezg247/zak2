...
def test_suspend_writes_rows() -> None:
    ...
    with patch("app.services.ops.sync_suspend.ts.require_token", return_value="tok"), patch(
        "app.services.ops.sync_suspend.latest_open_yyyymmdd", return_value="20260811"
    ), patch("app.services.ops.sync_suspend.ts.query", return_value=rows), patch(
        "app.services.ops.sync_suspend.save_job_run_meta"
    ):
        out = m.sync_suspend_daily(db)
