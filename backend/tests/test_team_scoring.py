...
def test_score_financial_pe() -> None:
    out = score_financial({"pe_ttm": 12, "pb": 1.5, "total_mv_yi": 500})
    assert out["score"] >= 50
    assert out["highlights"]


def test_score_risk_vol() -> None:
    low = score_risk({"volatility_annualized_pct": 18, "max_drawdown_pct": 12, "return_pct_60d": 5})
    high = score_risk({"volatility_annualized_pct": 55, "max_drawdown_pct": 45, "return_pct_60d": -20})
    assert low["score"] > high["score"]


def test_score_strategy_emotion() -> None:
    bad = score_strategy({"emotion_stage": "recession", "emotion_stage_label": "退潮", "allow_new_positions": False})
    good = score_strategy(
        {
            "ma_alignment": "均线多头排列",
            "signal": "buy",
...
def test_compute_team_scores_weighted() -> None:
    scores = compute_team_scores(
        {
            "financial": {"pe_ttm": 15},
            "risk": {"volatility_annualized_pct": 20, "max_drawdown_pct": 15},
